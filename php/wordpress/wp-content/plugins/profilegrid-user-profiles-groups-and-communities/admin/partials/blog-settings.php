  ...
  <form name="pm_user_settings" id="pm_user_settings" method="post">
    ...
    <div class="childfieldsrow" id="pm_blog_html" style=" <?php  if($dbhandler->get_global_option_value('pm_enable_blog','1')==1){echo 'display:block;';} else { echo 'display:none;';} ?>">  
       ...
       <div class="childfieldsrow" id="pm_notify_admin_html" style=" <?php if($dbhandler->get_global_option_value('pm_blog_notification_admin',0)==1){echo 'display:block;';} else { echo 'display:none;';} ?>">
                ...
                {
                     $review_body = $dbhandler->get_global_option_value('pm_blog_notification_email_body',esc_html__("Hello, <br /> {{user_login}} from {{group_name}} has published a new post titled {{post_name}}. You can view the post by {{post_link}}","profilegrid-user-profiles-groups-and-communities")); 
                }
                else
                {
                    $review_body = $dbhandler->get_global_option_value('pm_blog_notification_email_body',esc_html__("Hello, <br /> {{user_login}} from {{group_name}} has submitted a new post titled {{post_name}} and is pending approval. You can moderate the post by {{edit_post_link}}","profilegrid-user-profiles-groups-and-communities"));
                }
                ?>
